...
import allure
import pytest
from testdata.products_data import test_create_fund_check_required, test_create_products
...
class TestProducts:
    @pytest.mark.parametrize('title, data', test_create_fund_check_required)
    ...
    def test_create_fund_check_required(self, title, admin_driver, data):
        """
        创建Products，校验必填项
        """
        ...
        pp.input_fund_details(data['products_value'])
        logger.info('断言预期结果')
        assert pp.wait_ele_visible(data['checkpoint'])
